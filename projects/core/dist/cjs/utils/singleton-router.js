...
const evaluate_route_1 = require("./evaluate-route");
...
class SingletonRouter {
    ...
    canActivate(guard) {
        const currentPath = (0, normalize_path_1.normalizePath)(window.location.pathname);
        const { shouldActivate } = (0, evaluate_route_1.evaluateRoute)(currentPath, guard.path);
        this.guards.push({
            ...guard,
            type: 'active',
    ...
    evaluate(savedRoutes, newPath) {
        ...
        savedRoutes.forEach(route => {
            const result = (0, evaluate_route_1.evaluateRoute)(newPath, route.pathCaller());
    ...
    async evaluateGuards(path) {
        ...
        for (let i = 0; i < this.guards.length; i++) {
            ...
            if (guard.type === 'deactivate') {
                const { shouldDeactivate, shouldActivate, routeParams } = (0, evaluate_route_1.evaluateRoute)(path, guard.path);
        ...
        for (let i = 0; i < this.guards.length; i++) {
            ...
            if (guard.type === 'active') {
                const { shouldActivate, routeParams } = (0, evaluate_route_1.evaluateRoute)(path, guard.path);
    ...
    async navigate(path, options = {}) {
        ...
        const currentPath = (0, normalize_path_1.normalizePath)(pathname);
        const newPath = (0, normalize_path_1.normalizePath)(path);
        if (currentPath === newPath)
            return;
        this.currentRoute = newPath;
        const passed = await this.evaluateGuards((0, normalize_path_1.normalizePath)(path, true));
        if (this.guardRedirectUrls.length > 0) {
        ...
        await this.runEvaluate(this.savedRoutes, (0, normalize_path_1.normalizePath)(newPath, true));
